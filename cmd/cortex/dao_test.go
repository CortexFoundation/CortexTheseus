...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Ctxc instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		cortex := runCtxc(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		cortex.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "cortex", "chaindata")
	db, err := ctxcdb.NewLDBDatabase(path, 0, 0)
	if err != nil {
