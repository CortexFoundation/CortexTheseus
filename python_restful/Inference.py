...
class Inference:
    ...
    def loadModel(self, name, type="cpu", gpu_id=0):
        ...
        mod = mx.mod.Module(
            symbol=sym, context=mx.cpu(),
            label_names=None) if type == "cpu" else mx.mod.Module(
                symbol=sym, context=mx.gpu(gpu_id), label_names=None)
        mod.bind(
            for_training=False,
            data_shapes=[('data', (self.batchsize, 3, 224, 224))])
    ...
    def testSingleInference(self, model, data, t="cpu"):
        ...
        try:
            if model not in self.model_list.keys():
                mod = self.loadModel(
                    os.path.join(self.model_dir, model), type=t)
                self.model_list[model] = mod[0]
            return {
                "status": "ok",
                "info": self.predictSingle(data, self.model_list[model])[0]
            }
...
class InferHelper(object):
    def __init__(self, config, n_jobs=10):
        ...
        for i in range(n_jobs):
            ...
            self.workers.append(
                Process(target=self.__handler, args=(config, child_conn, i)))
