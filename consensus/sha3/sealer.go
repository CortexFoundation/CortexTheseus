...
func (sha3 *SHAThree) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if sha3.config.PowMode == ModeFake || sha3.config.PowMode == ModeFullFake {
		...
		default:
			//log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", sha3.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if sha3.shared != nil {
		return sha3.shared.Seal(chain, block, results, stop)
	}
	...
	if sha3.rand == nil {
		...
		sha3.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	sha3.lock.Unlock()
	...
	log.Info("#### Before Mine", "ThreadNumber", sha3.threads, "Txs", len(block.Transactions()))
	for i := 0; i < sha3.threads; i++ {
		pend.Add(1)
		var err error
		go func(id int, nonce uint64, err *error) {
			defer pend.Done()
			sha3.lock.Lock()
			*err = sha3.Mine(block, id, nonce, abort, sha3.resultCh)
			sha3.lock.Unlock()
		}(i, uint64(sha3.rand.Int63()), &err)
		if err != nil {
			return err
		}
	}
	...
	go func() {
		...
		case <-stop:
			...
			close(abort)
		case result = <-sha3.resultCh:
			// One of the threads found a block, abort all others
			select {
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", sha3.SealHash(block.Header()))
			}
			close(abort)
		case <-sha3.update:
			...
			close(abort)
			// pend.Wait()
			if err := sha3.Seal(chain, block, results, stop); err != nil {
				log.Error("Failed to restart sealing after update", "err", err)
			}
			// return sha3.Seal(chain, block, stop)
		}
...
func (sha3 *SHAThree) Verify(block Block, hashNoNonce common.Hash, shareDiff *big.Int, solution *types.BlockSolution) (bool, bool, int64) {
	...
	ok := sha3.SHAThreeVerifyHeader(hashNoNonce.Bytes(), block.Nonce(), solution, targetDiff)
	if !ok {
		//fmt.Println("invalid solution")
		return false, false, 0
	}
	sha3Hash := common.BytesToHash(sha3.Sha3Solution(solution))
...
func (sha3 *SHAThree) VerifySolution(hash []byte, nonce uint32, solution *types.BlockSolution, target big.Int) (bool, common.Hash) {
	return false, common.Hash{}
}

// remote starts a standalone goroutine to handle remote mining related stuff.
func (sha3 *SHAThree) remote() {
	var (
	...
	getWork := func() ([4]string, error) {
		...
		sealhash := sha3.SealHash(currentWork.Header())
		res[0] = sealhash.Hex()
	...
	submitWork := func(nonce types.BlockNonce, hash common.Hash, sol types.BlockSolution) bool {
		...
		header.Solution = sol
		if err := sha3.VerifySeal(nil, header); err != nil {
			log.Warn("Invalid proof-of-work submitted", "hash", hash, "err", err)
			return false
		}

		// Make sure the result channel is created.
		if sha3.resultCh == nil {
			log.Warn("SHAThree result channel is empty, submitted mining result is rejected")
			return false
		}
	...
	for {
		select {
		case <-sha3.exitCh:
			// Exit remote loop if sha3 is closed and return relevant error.
			log.Warn("SHAThree cycle remote sealer is exiting")
			return
		case block := <-sha3.workCh:
