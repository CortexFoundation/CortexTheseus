...
__kernel void Recovery(__constant const siphash_keys *sipkeys, __global ulong4 *buffer, __global int *indexes, __constant uint2* recoveredges) {
  ...
  if (lid < PROOFSIZE) {
	...
	for (int loop = 0; loop < loops; loop++) {
		...
		if (counter == FLUSHB-1) {
                    ...
                    for (int i = 0; i < nflush; i += TMPPERLL4){
                        destination[((ulong)(row * NX + col) * maxOut + cnt + i) / TMPPERLL4 + halfA/sizeof(ulong4)] = uint2_to_ulong4(tmp[col][i], tmp[col][i+1], tmp[col][i+2], tmp[col][i+3]);
		    }
                    for (int t = 0; t < newCount; t++) {
            ...
            for (int col = lid; col < NX; col += dim) {
                ...
                for (int i = 0; i < localIdx; i += TMPPERLL4) {
                    ...
                    destination[((ulong)(row * NX + col) * maxOut + cnt) / TMPPERLL4 + halfA/sizeof(ulong4)] = uint2_to_ulong4(tmp[col][i], tmp[col][i+1], tmp[col][i+2], tmp[col][i+3]);
                }
            }
        }
