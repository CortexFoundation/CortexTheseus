...
inline ulong4 uint2_to_ulong4(uint2 v){
	ulong l = v.x;
	l = l << 32;
	l = l | v.y;
	return (ulong4)(l, 0, 0, 0);
}
...
//    template<int maxOut, typename EdgeOut>
        __kernel void SeedA(__constant const siphash_keys *sipkeys, __global ulong4 * __restrict__ buffer, __global int * __restrict__ indexes, int maxOut) {
            ...
            for (int i = 0; i < loops; i++) {
                ...
                if (counter == FLUSHA-1) {
                    ...
                    for (int i = 0; i < nflush; i += TMPPERLL4)
                        ...
                        buffer[((long)(row * NX + col) * maxOut + cnt + i) / TMPPERLL4] = uint2_to_ulong4(tmp[row][i]);
            ...
            for (int row = lid; row < NX; row += dim) {
                ...
                for (int i = 0; i < localIdx; i += TMPPERLL4) {
                    ...
                    buffer[((long)(row * NX + col) * maxOut + cnt) / TMPPERLL4] = uint2_to_ulong4(tmp[row][i]);
                }
            }
        }

    __kernel void Seed2A(__constant const siphash_keys *sipkeys, __global ulong4 * __restrict__ buffer, __global int * __restrict__ indexes) {
        const int group = get_group_id(0);//blockIdx.x;
        const int dim = get_local_size(0);//blockDim.x;
        const int lid = get_local_id(0);//threadIdx.x;
        ...
        for (int i = 0; i < loops; i++) {
            ...
            if (localIdx >= 4) {
                ...
                for (int l = 0; l < nflush; l += 4)
                    ...
                    buffer[((long)(lid * NX + col) * DUCK_A_EDGES + cnt + l) / 4] = uint2_to_ulong4(tmp[lid][l]);
        ...
        if (localIdx > 0) {
            ...
            buffer[((long)(lid * NX + col) * DUCK_A_EDGES + cnt) / 4] = uint2_to_ulong4(tmp[lid][0]);
        }
    }

//    template<int maxOut, typename EdgeOut>
        __kernel void SeedB(__constant const siphash_keys *sipkeys, __global const uint2 * __restrict__ source, __global ulong4 * __restrict__ destination, __global const int * __restrict__ sourceIndexes, __global int * __restrict__ destinationIndexes, const int maxOut, const uint halfA, const uint halfE) {
            const int group = get_group_id(0);//blockIdx.x;
            const int dim = get_local_size(0);//blockDim.x;
            const int lid = get_local_id(0);//threadIdx.x;
            ...
            for (int loop = 0; loop < loops; loop++) {
                ...
                if (counter == FLUSHB-1) {
                    ...
                    for (int i = 0; i < nflush; i += TMPPERLL4)
                        ...
                        destination[((long)(row * NX + col) * maxOut + cnt + i) / TMPPERLL4 + halfA] = uint2_to_ulong4(tmp[col][i]);
                    for (int t = 0; t < newCount; t++) {
            ...
            for (int col = lid; col < NX; col += dim) {
                ...
                for (int i = 0; i < localIdx; i += TMPPERLL4) {
                    ...
                    destination[((long)(row * NX + col) * maxOut + cnt) / TMPPERLL4 + halfA] = uint2_to_ulong4(tmp[col][i]);
                }
            }
        }

/* #define BKTGRAN NX / 2 */
    __kernel void Seed2B(__global const uint2 * __restrict__ source, __global ulong4 * __restrict__ destination, __global const int * __restrict__ sourceIndexes, __global int * __restrict__ destinationIndexes, const int halfA, const int halfE) {
        const int group = get_group_id(0);//blockIdx.x;
        const int dim = get_local_size(0);//blockDim.x;
        const int lid = get_local_id(0);//threadIdx.x;
        //const int FLUSHB2 = 2 * FLUSHB;
            ...
            if (edgeIndex < bucketEdges) {
                ...
                if (localIdx >= 4) {
                    ...
                    for (int i = 0; i < nflush; i += 4)
                        ...
                        destination[((row * NX + lid) * DUCK_A_EDGES + cnt + i) / 4] = uint2_to_ulong4(tmp[lid][i]);
        ...
        if (localIdx > 0) {
            ...
            destination[((long)(row * NX + lid) * DUCK_A_EDGES + cnt) / 4 + halfA] = uint2_to_ulong4(tmp[lid][0]);
        }
    }

//    template<int maxIn, typename EdgeIn, int maxOut, typename EdgeOut>
        __kernel void Round(const int round, __constant const siphash_keys *sipkeys, __global const uint2 * __restrict__ source, __global uint2 * __restrict__ destination, __global const int * __restrict__ sourceIndexes, __global int * __restrict__ destinationIndexes, const int maxIn, const int maxOut) {
            const int group = get_group_id(0);//blockIdx.x;
            const int dim = get_local_size(0);//blockDim.x;
            const int lid = get_local_id(0);//threadIdx.x;
...
//    template<int maxIn, int maxOut>
        __kernel void Round2(__global const uint2 * __restrict__ source, __global uint2 * __restrict__ destination, __global const int * __restrict__ sourceIndexes, __global int * __restrict__ destinationIndexes, const int maxIn, const int maxOut) {
            const int group = get_group_id(0);//blockIdx.x;
            const int dim = get_local_size(0);//blockDim.x;
            const int lid = get_local_id(0);//threadIdx.x;
           // const static int COUNTERWORDS = NZ / 16; // 16 2-bit counters per 32-bit word
...
//    template<int maxIn>
        __kernel void Tail(__global const uint2 *source, __global uint2 *destination, __global const int *sourceIndexes, __global int *destinationIndexes, const int maxIn) {
      const int lid = get_local_id(0);//threadIdx.x;
      const int group = get_group_id(0);//blockIdx.x;
      const int dim = get_local_size(0);//blockDim.x;
...
//    template<int maxIn>
        __kernel void Tail2(__global const uint2 *source, __global uint2 *destination, __global const int *sourceIndexes, __global int *destinationIndexes, const int maxIn) {
      const int lid = get_local_id(0);//threadIdx.x;
      const int group = get_group_id(0);//blockIdx.x;
      const int dim = get_local_size(0);//blockDim.x;
      int myEdges = sourceIndexes[group];
