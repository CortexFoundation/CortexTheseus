...
func (r *replayer) DeleteRange(start, end []byte) {
	...
	if rangeDeleter, ok := r.writer.(ctxcdb.KeyValueRangeDeleter); ok {
		r.failure = rangeDeleter.DeleteRange(start, end)
	} else {
		r.failure = fmt.Errorf("ctxcdb.KeyValueWriter does not implement DeleteRange")
	}
}
