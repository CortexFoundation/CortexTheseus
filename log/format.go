...
func TerminalFormat(usecolor bool) Format {
	return FormatFunc(func(r *Record) []byte {
		...
		if atomic.LoadUint32(&locationEnabled) != 0 {
			...
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[37m[%s|%s]\x1b[0m%s %s ", color, lvl, r.Time.Format(termTimeFormat), location, padding, r.Msg)
			} else {
				fmt.Fprintf(b, "%s[%s|%s]%s %s ", lvl, r.Time.Format(termTimeFormat), location, padding, r.Msg)
			}
		} else {
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[37m[%s]\x1b[0m %s ", color, lvl, r.Time.Format(termTimeFormat), r.Msg)
			} else {
