...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Cortex
...
func (ctxc *Cortex) startBloomHandlers(sectionSize uint64) {
			...
			for {
				...
				case request := <-ctxc.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(ctxc.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(ctxc.chainDb, task.Bit, section, head); err == nil {
