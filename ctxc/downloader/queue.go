...
type fetchRequest struct {
	...
	From    uint64          // [ctxc/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [ctxc/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [ctxc/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [ctxc/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [ctxc/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [ctxc/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [ctxc/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [ctxc/62] Result cache accumulating the completed headers
	headerProced    int                            // [ctxc/62] Number of headers already processed from the results
	headerOffset    uint64                         // [ctxc/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [ctxc/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [ctxc/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [ctxc/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [ctxc/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [ctxc/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [ctxc/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [ctxc/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [ctxc/63] Currently pending receipt retrieval operations
	//receiptDonePool  map[common.Hash]struct{}      // [ctxc/63] Set of the completed receipt fetches
