...
type downloadTester struct {
	...
	stateDb ctxcdb.Database // Database used by the tester for syncing from peers
	peerDb  ctxcdb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.downloader = New(0, tester.stateDb, trie.NewSyncBloom(1, tester.stateDb), new(event.TypeMux), tester, tester.dropPeer)
	return tester
}
...
func (dl *downloadTester) FastSyncCommitHead(hash common.Hash) error {
	...
	if block := dl.GetBlockByHash(hash); block != nil {
		_, err := trie.NewStateTrie(trie.TrieID(block.Root()), trie.NewDatabase(dl.stateDb))
		return err
	}
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", cortex.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", cortex.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", cortex.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", cortex.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want cortex.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", cortex.SyncProgress{
