...
//	fmt.Print(pm.blockchain.GetBlockByNumber(limit / 2))
//	tests := []struct {
...
//	blockchain, err := core.NewBlockChain(db, nil, config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
//	if err != nil {
//		t.Fatalf("failed to create new blockchain: %v", err)
//	}
//	pm, err := NewProtocolManager(config, syncmode, DefaultConfig.NetworkId, new(event.TypeMux), &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, cuckoo.NewFaker(), blockchain, db, 1, nil)
//	if err != nil {
...
//	peer, _ := newTestPeer("peer", ctxc63, pm, true)
//	defer peer.close()
//
//	if checkpoint {
...
//		t.Fatalf("failed to create new blockchain: %v", err)
//	}
//	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, evmux, &testTxPool{pool: make(map[common.Hash]*types.Transaction)}, pow, blockchain, db, 1, nil)
//	if err != nil {
//		t.Fatalf("failed to start test protocol manager: %v", err)
//	}
//	pm.Start(1000)
...
//		peer, _ := newTestPeer(fmt.Sprintf("peer %d", i), ctxc63, pm, true)
//		defer peer.close()
//
//		peers = append(peers, peer)
//	}
//	chain, _ := core.GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {})
...
//		t.Fatalf("failed to create new blockchain: %v", err)
//	}
//	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, new(event.TypeMux), new(testTxPool), engine, blockchain, db, 1, nil)
//	if err != nil {
//		t.Fatalf("failed to start test protocol manager: %v", err)
//	}
//	pm.Start(2)
...
//	// propagation
//	source, _ := newTestPeer("source", ctxc63, pm, true)
//	defer source.close()
//
//	sink, _ := newTestPeer("sink", ctxc63, pm, true)
//	defer sink.close()
//
