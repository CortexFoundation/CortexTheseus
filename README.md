...
Official golang implementation of the CortexTheseus protocol.

[![API Reference](
https://camo.githubusercontent.com/915b7be44ada53c290eb157634330494ebe3e30a/68747470733a2f2f676f646f632e6f72672f6769746875622e636f6d2f676f6c616e672f6764646f3f7374617475732e737667
)](https://godoc.org/github.com/CortexFoundation/CortexTheseus)
[![Go Report Card](https://goreportcard.com/badge/github.com/CortexFoundation/CortexTheseus)](https://goreportcard.com/report/github.com/CortexFoundation/CortexTheseus)
[![Travis](https://travis-ci.org/CortexFoundation/CortexTheseus.svg?branch=master)](https://travis-ci.org/CortexFoundation/CortexTheseus)
...
The CortexTheseus project comes with several wrappers/executables found in the `cmd` directory.
...
Specifying the `--testnet` flag however will reconfigure your Ctxc instance a bit:
...
for play-money and real-money. Unless you manually move accounts, Ctxc will by default correctly
...
```
docker run -d --name CortexFoundation-node -v /Users/alice/CortexFoundation:/root \
...
As a developer, sooner rather than later you'll want to start interacting with Ctxc and the Ethereum
network via your own programs and not manually through the console. To aid this, Ctxc has built-in
support for a JSON-RPC based APIs ([standard APIs](https://github.com/CortexFoundation/wiki/wiki/JSON-RPC) and
[Ctxc specific APIs](https://github.com/CortexFoundation/CortexTheseus/wiki/Management-APIs)). These can be
exposed via HTTP, WebSockets and IPC (unix sockets on unix based platforms, and named pipes on Windows).

The IPC interface is enabled by default and exposes all the APIs supported by Ctxc, whereas the HTTP
...
via HTTP, WS or IPC to a Ctxc node configured with the above flags and you'll need to speak [JSON-RPC](http://www.jsonrpc.org/specification)
...
With the genesis state defined in the above JSON file, you'll need to initialize **every** Ctxc node
...
With the bootnode online, it will display an [`enode` URL](https://github.com/CortexFoundation/wiki/wiki/enode-url-format)
that other nodes can use to connect to it and exchange peer information. Make sure to replace the
displayed IP address information (most probably `[::]`) with your externally accessible IP to get the
actual `enode` URL.

*Note: You could also use a full fledged Ctxc node as a bootnode, but it's the less recommended way.*

#### Starting up your member nodes

With the bootnode operational and externally reachable (you can try `telnet <ip> <port>` to ensure
it's indeed reachable), start every subsequent Ctxc node pointed to the bootnode for peer discovery
...
[EtherMining subreddit](https://www.reddit.com/r/EtherMining/) and the [Genoil miner](https://github.com/Genoil/cpp-CortexFoundation)
repository.

In a private network setting however, a single CPU miner instance is more than enough for practical
purposes as it can produce a stable stream of blocks at the correct intervals without needing heavy
resources (consider running on a single thread, no need for multiple ones either). To start a Ctxc
...
If you'd like to contribute to CortexTheseus, please fork, fix, commit and send a pull request
for the maintainers to review and merge into the main code base. If you wish to submit more
complex changes though, please check up with the core devs first on [our gitter channel](https://gitter.im/CortexFoundation/CortexTheseus)
...
Please see the [Developers' Guide](https://github.com/CortexFoundation/CortexTheseus/wiki/Developers'-Guide)
for more details on configuring your environment, managing project dependencies and testing procedures.

## License

The CortexTheseus library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html), also
included in our repository in the `COPYING.LESSER` file.

The CortexTheseus binaries (i.e. all code inside of the `cmd` directory) is licensed under the
