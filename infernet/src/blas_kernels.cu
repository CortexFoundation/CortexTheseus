...
__global__ void int_scale_add_bias_kernel(int *output, char *input, char *scales, char *biases, int n, int size)
{
    int offset = blockIdx.x * blockDim.x + threadIdx.x;
    int filter = blockIdx.y;
    int batch = blockIdx.z;
...
void int_scale_add_bias_gpu(int *output, char *input, char *scales, char *biases, int batch, int n, int size)
{
    dim3 dimGrid((size-1)/BLOCK + 1, n, batch);
    dim3 dimBlock(BLOCK, 1, 1);

    int_scale_add_bias_kernel<<<dimGrid, dimBlock>>>(output, input, scales, biases, n, size);
    check_error(cudaPeekAtLastError());
}
...
__global__ void int_add_bias_kernel(char *output, char *biases, int batch, int n, int size)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (index >= n*size*batch) return;
    int i = index % size;
...
__global__ void int_fill_kernel(int N, char ALPHA, char *X, int INCX)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) X[i*INCX] = ALPHA;
}

__global__ void int32_fill_kernel(int N, int ALPHA, int *X, int INCX)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) X[i*INCX] = ALPHA;
}
...
__global__ void int_copy_kernel(int N, char *X, int OFFX, int INCX, char *Y, int OFFY, int INCY)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) Y[i*INCY + OFFY] = X[i*INCX + OFFX];
}
...
extern "C" void int_copy_gpu(int N, char * X, int INCX, char * Y, int INCY)
{
    int_copy_gpu_offset(N, X, 0, INCX, Y, 0, INCY);
}

extern "C" void int_copy_gpu_offset(int N, char * X, int OFFX, int INCX, char * Y, int OFFY, int INCY)
{
    int_copy_kernel<<<cuda_gridsize(N), BLOCK>>>(N, X, OFFX, INCX, Y, OFFY, INCY);
    check_error(cudaPeekAtLastError());
}
...
extern "C" void int_fill_gpu(int N, char ALPHA, char * X, int INCX)
{
    int_fill_kernel<<<cuda_gridsize(N), BLOCK>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}

extern "C" void int32_fill_gpu(int N, int ALPHA, int * X, int INCX)
{
    int32_fill_kernel<<<cuda_gridsize(N), BLOCK>>>(N, ALPHA, X, INCX);
    check_error(cudaPeekAtLastError());
}
...
__global__ void int_shortcut_kernel(int size, int minw, int minh, int minc, int stride, int sample, int batch, int w1, int h1, int c1, char *add, int w2, int h2, int c2, char s1, char s2, char *out)
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= size) return;
    int i = id % minw;
    ...
    out[out_index] = (out[out_index] >> s1) + (add[add_index] >> s2);
   
}

extern "C" void int_shortcut_gpu(int batch, int w1, int h1, int c1, char *add, int w2, int h2, int c2, char s1, char s2, char *out)
{
    int minw = (w1 < w2) ? w1 : w2;
    int minh = (h1 < h2) ? h1 : h2;
    int minc = (c1 < c2) ? c1 : c2;
    ...
    int size = batch * minw * minh * minc;
    int_shortcut_kernel<<<cuda_gridsize(size), BLOCK>>>(size, minw, minh, minc, stride, sample, batch, w1, h1, c1, add, w2, h2, c2, s1, s2, out);
    check_error(cudaPeekAtLastError());
}
