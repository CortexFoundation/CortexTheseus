 ...
 * \file pack_args.h
 * \brief Utility to pack CVMArgs to other type-erased fution calling convention.
 *
 ...
 */
#ifndef CVM_RUNTIME_PACK_ARGS_H_
#define CVM_RUNTIME_PACK_ARGS_H_
 ...
 *
 * \param f with signiture (CVMArgs args, CVMRetValue* rv, void* void_args)
 ...
 */
template<typename F>
inline PackedFunc PackFuncVoidAddr(F f, const std::vector<CVMType>& arg_types);
/*!
 * \brief Create a packed function that from function only packs buffer arguments.
 *
 * \param f with signiture (CVMArgs args, CVMRetValue* rv, ArgUnion* pack_args)
 * \param arg_types The arguments type information.
 * \tparam F the function type
 *
 * \return The wrapped packed function.
 */
template<typename F>
inline PackedFunc PackFuncNonBufferArg(F f, const std::vector<CVMType>& arg_types);
/*!
 * \brief Create a packed function that from function that takes a packed arguments.
 *
 * \param f with signature (CVMArgs args, CVMRetValue* rv, void* pack_args, size_t nbytes)
 ...
 */
template<typename F>
inline PackedFunc PackFuncPackedArg(F f, const std::vector<CVMType>& arg_types);
/*!
 ...
 */
inline size_t NumBufferArgs(const std::vector<CVMType>& arg_types);
...
inline ArgConvertCode GetArgConvertCode(CVMType t) {
  CHECK_EQ(t.lanes, 1U)
...
inline PackedFunc PackFuncVoidAddr_(F f, const std::vector<ArgConvertCode>& codes) {
  ...
  auto ret = [f, codes, num_args](CVMArgs args, CVMRetValue* ret) {
...
inline PackedFunc PackFuncNonBufferArg_(
  ...
  int num_args = static_cast<int>(codes.size());
  auto ret = [f, codes, base, num_args](CVMArgs args, CVMRetValue* ret) {
    TempArray<ArgUnion, N> holder_(num_args);
    ArgUnion* holder = holder_.data();
    for (int i = 0; i < num_args; ++i) {
      switch (codes[i]) {
...
inline PackedFunc PackFuncPackedArg_(
  ...
  auto ret = [f, codes, num_args](CVMArgs args, CVMRetValue* ret) {
    ...
    int32_t* ptr = pack;
    static_assert(sizeof(CVMValue) == 8, "invariant");
    ...
    for (int i = 0; i < num_args; ++i) {
      switch (codes[i]) {
        ...
        case INT64_TO_INT64:
        case FLOAT64_TO_FLOAT64: {
          std::memcpy(ptr, &args.values[i], sizeof(CVMValue));
          ptr += 2;
          break;
        }
        case INT64_TO_INT32: {
...
#endif  // CVM_RUNTIME_PACK_ARGS_H_
