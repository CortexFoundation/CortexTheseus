...
__device__ char int_linear_activate_kernel(char x){return x;}
__device__ char int_relu_activate_kernel(char x){return x*(x>0);}

__device__ char int_activate_kernel(char x, ACTIVATION a)
{
    switch(a){
        case LINEAR:
            return int_linear_activate_kernel(x);
        case RELU:
            return int_relu_activate_kernel(x);
    }
    return 0;
}
...
__global__ void int_activate_array_kernel(char *x, int n, ACTIVATION a)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < n) x[i] = int_activate_kernel(x[i], a);
}
...
extern "C" void int_activate_array_gpu(char *x, int n, ACTIVATION a) 
{
    int_activate_array_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, a);
    check_error(cudaPeekAtLastError());
}
