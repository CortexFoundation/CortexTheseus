...
class IndexedGraph {
 public:
  ...
  struct Node {
    ...
    std::weak_ptr<cvm::Node> weak_ref;
  };
   ...
   * \return the unique index.
   */
  inline uint32_t entry_id(const cvm::NodeEntry& e) const {
   ...
   */
  inline uint32_t node_id(const cvm::Node* node) const {
   ...
   * \return const reference to the corresponding IndexedGraph::Node
   */
  inline const Node& operator[](const cvm::Node* node) const {
  ...
  inline bool exist(const cvm::Node* node) const {
 ...
 private:
  ...
  std::unordered_map<const cvm::Node*, uint32_t> node2index_;
...
inline const T& Graph::GetAttr(const std::string& attr_name) const {
  ...
  return cvm::unsafe_get<T>(*it->second);
}
...
inline T Graph::MoveCopyAttr(const std::string& attr_name) {
  ...
  attrs.erase(it);
  if (sptr.unique()) {
    return std::move(cvm::get<T>(*sptr));
  } else {
    return cvm::get<T>(*sptr);
  }
}
