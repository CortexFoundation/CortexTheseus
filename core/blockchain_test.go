...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (ctxcdb.Database, *BlockChain, error) {
	var (
		db  = rawdb.NewMemoryDatabase()
		gen = new(Genesis)
	)
	gen.Supply = params.CTXC_INIT
	genesis := gen.MustCommit(db)
	// Initialize a fresh chain with only a genesis block
	blockchain, _ := NewBlockChain(db, nil, params.AllCuckooProtocolChanges, engine, vm.Config{}, nil, nil)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int)) {
	...
	db, blockchain2, err := newCanonical(cuckoo.NewFaker(), i, full)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.CurrentBlock(), n, cuckoo.NewFaker(), db, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.CurrentHeader(), n, cuckoo.NewFaker(), db, forkSeed)
...
func TestLastBlock(t *testing.T) {
	_, blockchain, err := newCanonical(cuckoo.NewFaker(), 0, true)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	blocks := makeBlockChain(blockchain.CurrentBlock(), 1, cuckoo.NewFullFaker(), blockchain.db, 0)
...
//	_, processor, err := newCanonical(cuckoo.NewFaker(), length, full)
//	if err != nil {
...
//	db, blockchain, err := newCanonical(cuckoo.NewFaker(), 10, full)
//	if err != nil {
//		t.Fatalf("failed to make new canonical chain: %v", err)
//	}
//	defer blockchain.Stop()
...
//	if full {
//		chain := makeBlockChain(blockchain.CurrentBlock(), 5, cuckoo.NewFaker(), db, forkSeed)[1:]
//		if err := testBlockChainImport(chain, blockchain); err == nil {
//			t.Errorf("broken block chain not reported")
//		}
//	} else {
//		chain := makeHeaderChain(blockchain.CurrentHeader(), 5, cuckoo.NewFaker(), db, forkSeed)[1:]
...
//	db, blockchain, err := newCanonical(cuckoo.NewFaker(), 0, full)
//	if err != nil {
//		t.Fatalf("failed to create pristine chain: %v", err)
//	}
//	defer blockchain.Stop()
...
//	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), cuckoo.NewFaker(), db, len(first), func(i int, b *BlockGen) {
//		b.OffsetTime(first[i])
//	})
//	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.CurrentBlock(), cuckoo.NewFaker(), db, len(second), func(i int, b *BlockGen) {
...
//	if full {
//		blocks := makeBlockChain(blockchain.CurrentBlock(), 3, cuckoo.NewFaker(), db, 10)
//
...
//		_, err = blockchain.InsertChain(blocks)
//	} else {
//		headers := makeHeaderChain(blockchain.CurrentHeader(), 3, cuckoo.NewFaker(), db, 10)
//
...
//	headers := makeHeaderChain(blockchain.CurrentHeader(), 4, cuckoo.NewFaker(), db, 10)
//	blocks := makeBlockChain(blockchain.CurrentBlock(), 4, cuckoo.NewFaker(), db, 10)
//
//	if full {
//		if _, err = blockchain.InsertChain(blocks); err != nil {
...
//	ncm, err := NewBlockChain(blockchain.db, nil, blockchain.chainConfig, cuckoo.NewFaker(), vm.Config{}, nil, nil)
//	if err != nil {
...
//		)
//		if full {
//			blocks := makeBlockChain(blockchain.CurrentBlock(), i, cuckoo.NewFaker(), db, 0)
//
//			failAt = rand.Int() % len(blocks)
//			failNum = blocks[failAt].NumberU64()
//
//			blockchain.engine = cuckoo.NewFakeFailer(failNum)
//			failRes, err = blockchain.InsertChain(blocks)
//		} else {
//			headers := makeHeaderChain(blockchain.CurrentHeader(), i, cuckoo.NewFaker(), db, 0)
//
...
//	)
//	blocks, receipts := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), gendb, 1024, func(i int, block *BlockGen) {
...
//	archive, _ := NewBlockChain(archiveDb, nil, gspec.Config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
//	defer archive.Stop()
//
...
//	fast, _ := NewBlockChain(fastDb, nil, gspec.Config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
//	defer fast.Stop()
//
...
//	ancient, _ := NewBlockChain(ancientDb, nil, gspec.Config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
//	defer ancient.Stop()
//
...
//	)
//	height := uint64(1024)
//	blocks, receipts := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), gendb, int(height), nil)
//
//	// makeDb creates a db instance for testing.
//	makeDb := func() (ctxcdb.Database, func()) {
...
//	defer delfn()
//	light, _ := NewBlockChain(lightDb, nil, gspec.Config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
...
//
//	chain, _ := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 3, func(i int, gen *BlockGen) {
//		switch i {
//		case 0:
...
//	blockchain, _ := NewBlockChain(db, nil, gspec.Config, cuckoo.NewFaker(), vm.Config{}, nil, nil)
...
//	chain, _ = GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 5, func(i int, gen *BlockGen) {
//		switch i {
//		case 0:
...
//	chain, _ := GenerateChain(params.TestChainConfig, genesis, cuckoo.NewFaker(), db, 2, func(i int, gen *BlockGen) {
//		if i == 1 {
...
//	}
//
//	chain, _ = GenerateChain(params.TestChainConfig, genesis, cuckoo.NewFaker(), db, 3, func(i int, gen *BlockGen) {})
...
//	sideChain, _ := GenerateChain(params.TestChainConfig, genesis, cuckoo.NewFaker(), db, 2, func(i int, gen *BlockGen) {
//		if i == 1 {
//			tx, err := types.SignTx(types.NewContractCreation(gen.TxNonce(addr1), new(big.Int), 1000000, new(big.Int), logCode), signer, key1)
//			if err != nil {
//				t.Fatalf("failed to create tx: %v", err)
...
//	newBlocks, _ := GenerateChain(params.TestChainConfig, sideChain[len(sideChain)-1], cuckoo.NewFaker(), db, 1, func(i int, gen *BlockGen) {})
//	if _, err := blockchain.InsertChain(newBlocks); err != nil {
//		t.Fatalf("failed to insert forked chain: %v", err)
//	}
//	checkLogEvents(t, newLogCh, rmLogsCh, 1, 0)
...
//	}
//
//	replacementBlocks, _ := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 4, func(i int, gen *BlockGen) {
...
//	defer blockchain.Stop()
//
//	chain, _ := GenerateChain(blockchain.chainConfig, blockchain.genesisBlock, cuckoo.NewFaker(), blockchain.db, 10, func(i int, gen *BlockGen) {})
//
...
//	defer blockchain.Stop()
//
//	blocks, _ := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 4, func(i int, block *BlockGen) {
//		var (
...
//	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], cuckoo.NewFaker(), db, 4, func(i int, block *BlockGen) {
//		var (
//			tx      *types.Transaction
//			err     error
//			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
//	defer blockchain.Stop()
//
//	blocks, _ := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), db, 3, func(i int, block *BlockGen) {
//		var (
//			tx     *types.Transaction
//			err    error
//			signer = types.NewEIP155Signer(gspec.Config.ChainID)
...
//	)
//	height := uint64(128)
//	blocks, receipts := GenerateChain(gspec.Config, genesis, cuckoo.NewFaker(), gendb, int(height), func(i int, block *BlockGen) {
//		tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), common.Address{0x00}, big.NewInt(1000), params.TxGas, nil, nil), signer, key)
//		if err != nil {
//			panic(err)
//		}
//		block.AddTx(tx)
//	})
//	blocks2, _ := GenerateChain(gspec.Config, blocks[len(blocks)-1], cuckoo.NewFaker(), gendb, 10, nil)
//
...
//	l := uint64(0)
//	chain, err := NewBlockChain(ancientDb, nil, params.TestChainConfig, cuckoo.NewFaker(), vm.Config{}, nil, &l)
//	if err != nil {
//		t.Fatalf("failed to create tester chain: %v", err)
//	}
//	headers := make([]*types.Header, len(blocks))
