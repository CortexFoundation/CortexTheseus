...
func (s *stepCounter) CaptureState(env *vm.CVM, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, rStack *vm.ReturnStack, rData []byte, contract *vm.Contract, depth int, err error) error {
	s.steps++
...
func (s *stepCounter) CaptureFault(env *vm.CVM, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, rStack *vm.ReturnStack, contract *vm.Contract, depth int, err error) error {
	return nil
}
...
func TestJumpSub1024Limit(t *testing.T) {
	...
	_, _, err := Call(address, nil, &Config{State: state,
		GasLimit:    20000,
		ChainConfig: params.AllCuckooProtocolChanges,
		CVMConfig: vm.Config{
			ExtraEips: []int{2315},
			Debug:     true,
