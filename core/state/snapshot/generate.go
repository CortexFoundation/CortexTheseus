...
func generateSnapshot(diskdb ctxcdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, wiper chan struct{}) *diskLayer {
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	for accIt.Next() {
		...
		data := AccountRLP(acc.Nonce, acc.Balance, acc.Root, acc.CodeHash, acc.Upload, acc.Num)
		...
		if batch.ValueSize() > ctxcdb.IdealBatchSize || abort != nil {
