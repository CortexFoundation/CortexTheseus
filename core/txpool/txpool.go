...
func New(config Config, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool.reserver = reserver.NewHandle(0)
	...
	if !config.NoLocals && config.Journal != "" {
		...
		if err := pool.journal.setupWriter(); err != nil {
...
func (pool *TxPool) local() map[common.Address]types.Transactions {
	...
	for addr := range pool.locals.accounts {
		...
		if queued, ok := pool.queue.get(addr); ok {
...
func (pool *TxPool) validateTxBasics(tx *types.Transaction, head *types.Header, local bool) error {
	...
	if pool.currentHead.Load().GasLimit < tx.Gas() {
		return ErrGasLimit
	}
	...
	if tx != nil && tx.To() != nil && tx.Value().Sign() == 0 {
		pool.mu.Lock()
		uploading = pool.currentState.Uploading(*tx.To())
		pool.mu.Unlock()
	}
	intrGas, err := core.IntrinsicGas(tx.Data(), tx.To() == nil, uploading, true, pool.istanbul.Load())
	if err != nil {
		return err
	}
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	if uint64(pool.all.Slots()+numSlots(tx)) > pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		for _, tx := range drop {
			...
			underpricedTxMeter.Mark(1)
			dropped := pool.removeTx(tx.Hash(), false, true)
...
func (pool *TxPool) addTxs(txs []*types.Transaction, local, sync bool) []error {
	...
	for i, tx := range txs {
		...
		if err := pool.validateTxBasics(tx, pool.currentHead.Load(), local); err != nil {
			errs[i] = err
			invalidTxMeter.Mark(1)
			continue
		}
...
func (pool *TxPool) removeTx(hash common.Hash, outofbound bool, unreserve bool) int {
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(pool.currentState.GetBalance(addr), pool.currentHead.Load().GasLimit)
