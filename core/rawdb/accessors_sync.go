...
func ReadSkeletonSyncStatus(db ctxcdb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db ctxcdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db ctxcdb.KeyValueWriter) {
...
func ReadSkeletonHeader(db ctxcdb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db ctxcdb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db ctxcdb.KeyValueWriter, number uint64) {
