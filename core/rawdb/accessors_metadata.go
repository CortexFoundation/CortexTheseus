...
func ReadDatabaseVersion(db ctxcdb.KeyValueReader) *uint64 {
	var version uint64
...
func ReadChainConfig(db ctxcdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db ctxcdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db ctxcdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db ctxcdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
...
func UpdateUncleanShutdownMarker(db ctxcdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
