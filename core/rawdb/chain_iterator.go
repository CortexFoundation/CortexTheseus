...
func InitDatabaseFromFreezer(db ctxcdb.Database) {
...
func iterateTransactions(db ctxcdb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db ctxcdb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db ctxcdb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
