...
func (f *MemoryFreezer) ReadAncients(fn func(ctxcdb.AncientReaderOp) error) (err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *MemoryFreezer) ModifyAncients(fn func(ctxcdb.AncientWriteOp) error) (writeSize int64, err error) {
	f.lock.Lock()
	defer f.lock.Unlock()
