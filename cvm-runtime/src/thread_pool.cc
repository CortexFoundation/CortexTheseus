...
class ParallelLauncher {
 public:
  ...
  void Init(FCVMParallelLambda flambda,
            void* cdata,
            int num_task,
            bool need_sync) {
...
class ThreadPool {
 public:
  ThreadPool(): num_workers_(cvm::runtime::threading::MaxConcurrency()) {
    for (int i = 0; i < num_workers_; ++i) {
      // The SpscTaskQueue only hosts ONE item at a time
      queues_.emplace_back(std::unique_ptr<SpscTaskQueue>(new SpscTaskQueue()));
    }
    threads_ = std::unique_ptr<cvm::runtime::threading::ThreadGroup>(
        new cvm::runtime::threading::ThreadGroup(
  ...
  int Launch(FCVMParallelLambda flambda,
             void* cdata,
             int num_task,
             int need_sync) {
    ...
    if (exclude_worker0_) {
      CVMParallelGroupEnv* penv = &(tsk.launcher->env);
 ...
 private:
  ...
  void RunWorker(int worker_id) {
    ...
    while (queue->Pop(&task)) {
      ...
      CVMParallelGroupEnv* penv = &(task.launcher->env);
  ...
  std::unique_ptr<cvm::runtime::threading::ThreadGroup> threads_;
};

CVM_REGISTER_GLOBAL("runtime.config_threadpool")
...
int CVMBackendParallelLaunch(
    FCVMParallelLambda flambda,
    void* cdata,
    int num_task) {
  int res = cvm::runtime::ThreadPool::ThreadLocal()->Launch(
      flambda, cdata, num_task, 1);
  return res;
}

int CVMBackendParallelBarrier(int task_id, CVMParallelGroupEnv* penv) {
  using cvm::runtime::kSyncStride;
