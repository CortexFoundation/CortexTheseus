...
static void SubgraphSanityCheck(const std::vector<std::shared_ptr<Symbol>> &subgraphs) {
  std::vector<const std::vector<cvm::NodeEntry>*> curr_level;
  std::vector<const std::vector<cvm::NodeEntry>*> next_level;
  std::unordered_map<cvm::Node*, uint32_t> node2level;
...
IndexedGraph::IndexedGraph(const Graph &g) {
  ...
  const NodeEntry* iptr = utils::BeginPtr(input_entries_);
  ...
  const uint32_t* cptr = utils::BeginPtr(control_deps_);
  for (size_t nid = 0; nid < nodes_.size(); ++nid) {
