...
cvm::NodePtr CreateLayoutTransformNode(const Layout& src,
                                        const Layout& dst) {
  static const cvm::Op* trans_op = cvm::Op::Get("__layout_transform__");
  static int count = 0;
  cvm::NodePtr n = cvm::Node::Create();
  n->attrs.op = trans_op;
 ...
 */
cvm::Graph CorrectLayout(cvm::Graph src) {
  static auto& op_correct_layout =
    cvm::Op::GetAttr<FCorrectLayout>("FCorrectLayout");

  const IndexedGraph& idx = src.indexed_graph();
  std::vector<cvm::NodePtr> mirror_vec(idx.num_nodes(), nullptr);
  ...
  for (uint32_t nid = 0; nid < idx.num_nodes(); ++nid) {
    ...
    cvm::NodePtr new_node = cvm::Node::Create();
    *new_node = *(inode.source);
    ...
    for (uint32_t i = 0; i < inode.inputs.size(); ++i) {
      ...
      const cvm::NodePtr& in = mirror_vec[e.node_id];
      new_node->inputs[i] = cvm::NodeEntry{in, e.index, e.version};
  ...
  std::vector<cvm::NodeEntry> outputs;
  for (const auto& e : idx.outputs()) {
    outputs.emplace_back(cvm::NodeEntry{mirror_vec[e.node_id], e.index, e.version});
  }
