...
class GraphAllocator {
 public:
  ...
  explicit GraphAllocator(const IndexedGraph* idx, const size_t match_range) : idx_(idx) {
    this->Init(match_range, utils::GetEnv("CVM_EXEC_NUM_TEMP", 1));
  }
...
Graph PlanMemory(Graph ret) {
  ...
  // Search the best CVM_EXEC_MATCH_RANGE parameter. This is turned off by default
  size_t min_allocated_bytes = -1;
  size_t max_match_range = utils::GetEnv("CVM_EXEC_MATCH_RANGE", 16);
  size_t min_match_range =
         utils::GetEnv("CVM_AUTO_SEARCH_MATCH_RANGE", false) ? 1 : max_match_range;
