 ...
 * \file cvm/runtime/packed_func.h
 * \brief Type-erased function used across CVM API.
 */
#ifndef CVM_RUNTIME_PACKED_FUNC_H_
#define CVM_RUNTIME_PACKED_FUNC_H_
...
// Whether use CVM runtime in header only mode.
#ifndef CVM_RUNTIME_HEADER_ONLY
#define CVM_RUNTIME_HEADER_ONLY 0
#endif
 ...
 *  It is the unified function function type of CVM.
 *  It corresponds to CVMFunctionHandle in C runtime API.
 */
class PackedFunc {
 public:
  /*!
   ...
   *   void MyPackedFunc(CVMArgs args, CVMRetValue* rv) {
...
class TypedPackedFunc<R(Args...)> {
 public:
   ...
   * Example usage:
   * \code
   * PackedFunc packed([](CVMArgs args, CVMRetValue *rv) {
   *   int x = args[0];
   *   *rv = x + 1;
   *  });
  ...
  /*!
   * \brief constructor from CVMRetValue
   * \param value The CVMRetValue
   */
  inline TypedPackedFunc(const CVMRetValue& value);  // NOLINT(*)
  /*!
   * \brief constructor from CVMArgValue
   * \param value The CVMArgValue
   */
...
/*! \brief Arguments into CVM functions. */
class CVMArgs {
 public:
...
/*!
 * \brief convert a string to CVM type.
 * \param s The string to be converted.
 * \return The corresponding cvm type.
 */
inline CVMType String2CVMType(std::string s);

/*!
 * \brief convert a CVM type to string.
 * \param t The type to be converted.
 * \return The corresponding cvm type in string.
 */
inline std::string CVMType2String(CVMType t);
...
/*!
 * \brief Type traits to mark if a class is cvm extension type.
 *
 * To enable extension type in C++ must be register () ed via marco.
 * CVM_REGISTER_EXT_TYPE(TypeName) after defining this with this traits.
 *
 * Extension class can be passed and returned via PackedFunc in all cvm runtime.
...
class ExtTypeVTable {
 public:
   ...
   * \return The registered vtable.
   */
  CVM_DLL static ExtTypeVTable* Get(int type_code);

 private:
  // Internal registration function.
  CVM_DLL static ExtTypeVTable* RegisterInternal(int type_code, const ExtTypeVTable& vt);
};
...
class CVMPODValue_ {
 public:
  ...
  operator void*() const {
    ...
    CVM_CHECK_TYPE_CODE(type_code_, kHandle);
  ...
  operator NDArray() const {
    ...
    CVM_CHECK_TYPE_CODE(type_code_, kNDArrayContainer);
    return NDArray(static_cast<NDArray::Container*>(value_.v_handle));
  }
  operator CVMContext() const {
    CVM_CHECK_TYPE_CODE(type_code_, kCVMContext);
    return value_.v_ctx;
  }
 ...
 protected:
  ...
  friend class CVMRetValue;
  CVMPODValue_() : type_code_(kNull) {}
  CVMPODValue_(CVMValue value, int type_code)
 ...
 *  Containing both type_code and CVMValue
 *
...
class CVMArgValue : public CVMPODValue_ {
 public:
   ...
   * \param type_code The type code.
   */
  CVMArgValue(CVMValue value, int type_code)
      : CVMPODValue_(value, type_code) {
  }
  ...
  using CVMPODValue_::operator CVMContext;

  // conversion operator.
  operator std::string() const {
    if (type_code_ == kCVMType) {
      return CVMType2String(operator CVMType());
    } else if (type_code_ == kBytes) {
      CVMByteArray* arr = static_cast<CVMByteArray*>(value_.v_handle);
      return std::string(arr->data, arr->size);
    } else {
      CVM_CHECK_TYPE_CODE(type_code_, kStr);
      return std::string(value_.v_str);
    }
  }
  operator CVMType() const {
    if (type_code_ == kStr) {
      return String2CVMType(operator std::string());
    }
    // None type
    if (type_code_ == kNull) {
      CVMType t;
    ...
    CVM_CHECK_TYPE_CODE(type_code_, kCVMType);
    return value_.v_type;
  }
  operator PackedFunc() const {
    if (type_code_ == kNull) return PackedFunc();
    CVM_CHECK_TYPE_CODE(type_code_, kFuncHandle);
  ...
  operator Module() const {
    CVM_CHECK_TYPE_CODE(type_code_, kModuleHandle);
    return *ptr<Module>();
  }
  ...
  inline operator cvm::Integer() const;
 ...
 *  Unlike CVMArgValue, which only holds reference and do not delete
 *  the underlying container during destruction.
 *
 *  CVMRetValue holds value and will manage the underlying containers
...
class CVMRetValue : public CVMPODValue_ {
 public:
   ...
   */
  CVMRetValue(CVMRetValue&& other)
      : CVMPODValue_(other.value_, other.type_code_) {
  ...
  using CVMPODValue_::operator CVMContext;
  using CVMPODValue_::operator NDArray;
  CVMRetValue(const CVMRetValue& other) : CVMPODValue_() {
    this->Assign(other);
  }
  ...
  CVMRetValue& operator=(CVMContext value) {
    this->SwitchToPOD(kCVMContext);
    value_.v_ctx = value;
    return *this;
  }
  CVMRetValue& operator=(CVMType t) {
    this->SwitchToPOD(kCVMType);
    value_.v_type = t;
    return *this;
  }
  ...
  CVMRetValue& operator=(const CVMRetValue& other) {  // NOLINT(*0
    this->Assign(other);
    return *this;
  }
  ...
  CVMRetValue& operator=(const T& other) {
   ...
   */
  void MoveToCHost(CVMValue* ret_value,
                   int* ret_type_code) {
  ...
  const CVMValue& value() const {
    CHECK(type_code_ != kNodeHandle &&
          ...
          type_code_ != kStr) << "CVMRetValue.value can only be used for POD data";
    return value_;
  }
  // NodeRef related extenstions: in cvm/packed_func_ext.h
  template<typename T,
           typename = typename std::enable_if<
             std::is_class<T>::value>::type>
  inline operator T() const;
  template<typename TNodeRef>
  inline TNodeRef AsNodeRef() const;
  inline CVMRetValue& operator=(const NodeRef& other);
  inline CVMRetValue& operator=(const NodePtr<Node>& other);
  // type related
...
inline const char* TypeCode2Str(int type_code) {
  switch (type_code) {
    ...
    case kCVMType: return "CVMType";
    case kCVMContext: return "CVMContext";
...
#ifndef _LIBCPP_SGX_NO_IOSTREAMS
inline std::ostream& operator<<(std::ostream& os, CVMType t) {  // NOLINT(*)
...
class CVMArgsSetter {
 public:
  CVMArgsSetter(CVMValue* values, int* type_codes)
  ...
  void operator()(size_t i, const CVMArgValue& value) const {
  ...
  void operator()(size_t i, CVMContext value) const {
    values_[i].v_ctx = value;
    type_codes_[i] = kCVMContext;
  }
  void operator()(size_t i, CVMType value) const {
    values_[i].v_type = value;
    type_codes_[i] = kCVMType;
  }
  ...
  void operator()(size_t i, const CVMByteArray& value) const {  // NOLINT(*)
  ...
  void operator()(size_t i, const CVMRetValue& value) const {  // NOLINT(*)
...
inline void unpack_call(const F& f, const CVMArgs& args, CVMRetValue* rv) {
...
struct CVMValueCast<T, TSrc, true, false> {
  static T Apply(const TSrc* self) {
...
template<typename T, typename TSrc>
struct CVMValueCast<T, TSrc, false, true> {
  static T Apply(const TSrc* self) {
...
template<typename T, typename>
inline void CVMArgsSetter::operator()(size_t i, const T& value) const {
...
inline PackedFunc Module::GetFunction(const std::string& name, bool query_imports) {
  return node_->GetFunction(name);
}
}  // namespace runtime
}  // namespace cvm
#endif  // CVM_RUNTIME_PACKED_FUNC_H_
