 ...
 * \file cvm/runtime/c_backend_api.h
 * \brief CVM runtime backend API.
 *
 *  The functions defined in this header are intended to be
 *  used by compiled cvm operators, usually user do not need to use these
 *  function directly.
 */
#ifndef CVM_RUNTIME_C_BACKEND_API_H_
#define CVM_RUNTIME_C_BACKEND_API_H_
 ...
 *  The user do should not call CVMFuncFree on func.
 *
 ...
 */
CVM_DLL int CVMBackendGetFuncFromEnv(void* mod_node,
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
CVM_DLL int CVMBackendRegisterSystemLibSymbol(const char* name, void* ptr);
 ...
 */
CVM_DLL void* CVMBackendAllocWorkspace(int device_type,
                                       int device_id,
                                       uint64_t nbytes,
                                       int dtype_code_hint,
                                       int dtype_bits_hint);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 *
 * \sa CVMBackendAllocWorkspace
 */
CVM_DLL int CVMBackendFreeWorkspace(int device_type,
                                    int device_id,
                                    void* ptr);

/*!
 * \brief Environment for CVM parallel task.
 */
typedef struct {
  /*!
 ...
 */
typedef int (*FCVMParallelLambda)(
    int task_id, CVMParallelGroupEnv* penv, void* cdata);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
CVM_DLL int CVMBackendParallelLaunch(FCVMParallelLambda flambda,
                                     void* cdata,
                                     int num_task);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
CVM_DLL int CVMBackendParallelBarrier(int task_id, CVMParallelGroupEnv* penv);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
CVM_DLL int CVMBackendRunOnce(void** handle,
                              int (*f)(void*),
                              void *cdata,
                              int nbytes);
...
#endif
#endif  // CVM_RUNTIME_C_BACKEND_API_H_
