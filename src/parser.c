...
int_convolutional_layer int_parse_convolutional(list *options, size_params params)
{
    int n = option_find_int(options, "filters",1);
    int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
...
layer int_parse_connected(list *options, size_params params)
{
    int output = option_find_int(options, "output",1);
    char *activation_s = option_find_str(options, "activation", "logistic");
    ACTIVATION activation = get_activation(activation_s);
...
int_maxpool_layer int_parse_maxpool(list *options, size_params params)
{
    int stride = option_find_int(options, "stride",1);
    int size = option_find_int(options, "size",stride);
    int padding = option_find_int_quiet(options, "padding", size-1);
    ...
    int_maxpool_layer layer = make_int_maxpool_layer(batch,h,w,c,size,stride,padding);
    return layer;
}
...
layer int_parse_activation(list *options, size_params params)
{
    char *activation_s = option_find_str(options, "activation", "linear");
    ACTIVATION activation = get_activation(activation_s);

    layer l = make_int_activation_layer(params.batch, params.inputs, activation);
...
network *int_parse_network_cfg(char *filename)
{
    list *sections = read_cfg(filename);
    node *n = sections->front;
    if(!n) error("Config file has no sections");
    ...
    while(n){
        ...
        LAYER_TYPE lt = string_to_layer_type(s->type);
        if(lt == CONVOLUTIONAL){
            l = int_parse_convolutional(options, params);
        }else if(lt == ACTIVE){
            l = int_parse_activation(options, params);
        }else if(lt == CONNECTED){
            l = int_parse_connected(options, params);
        }else if(lt == BATCHNORM){
    ...
    if(net->layers[net->n-1].truths) net->truths = net->layers[net->n-1].truths;
    net->output = out.output;
    net->input = calloc(net->inputs*net->batch, sizeof(char));
    net->truth = calloc(net->truths*net->batch, sizeof(char));
#ifdef GPU
    net->output_gpu = out.output_gpu;
    net->input_gpu = (float*)int_cuda_make_array((char*)net->input, net->inputs*net->batch);
    net->truth_gpu = (float*)int_cuda_make_array((char*)net->truth, net->truths*net->batch);
#endif
    if(workspace_size){
       
#ifdef GPU
        if(gpu_index >= 0){
            net->workspace = (float*)int_cuda_make_array(NULL, (workspace_size-1)/sizeof(char)+1);
        }else {
            net->workspace = calloc(1, workspace_size);
        }
#else
