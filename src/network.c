...
{
    network orig = *net;
    net->input = (float*)input;
    net->truth = 0;
    net->train = 0;
    net->delta = 0;
    int_forward_network_gpu(net);
    char *out = (char*)net->output;
    *net = orig;
    return out;
}
...
void int_forward_network_gpu(network *netp)
{
    network net = *netp;
    cuda_set_device(net.gpu_index);
    int_cuda_push_array((char*)net.input_gpu, (char*)net.input, net.inputs*net.batch);
    if(net.truth){
        int_cuda_push_array((char*)net.truth_gpu, (char*)net.truth, net.truths*net.batch);
    }
...
void int_pull_network_output(network *net)
{
    layer l = get_network_output_layer(net);
    int_cuda_pull_array((char*)l.output_gpu, (char*)l.output, l.outputs*l.batch);
}
