...
// JSONNode represents an cvm::Node in JSON
struct JSONNode;
// JSONGraph represents an cvm::Graph or cvm::Symbol in JSON
struct JSONGraph;
...
std::shared_ptr<Symbol> JSONGraph2Symbol(const JSONGraph &jgraph, bool no_parse) {
  for (const JSONNode &n : jgraph.nodes) {
    ...
    for (const JSONGraph &subgraph : n.subgraphs) {
      ...
      // incubator-mxnet/src/cvm/legacy_json_util.cc:UpgradeJSON_Parse
...
CVMUTIL_JSON_ENABLE_ANY(std::string, str);
CVMUTIL_JSON_ENABLE_ANY(std::vector<int>, list_int);
CVMUTIL_JSON_ENABLE_ANY(std::vector<std::string>, list_str);
