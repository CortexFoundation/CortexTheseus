...
inline void UpdateNodeVersion(Node *n) {
  ...
  for (NodeEntry& e : n->inputs) {
    if (e.node->is_variable()) {
      e.version = cvm::get<VariableParam>(e.node->attrs.parsed).version;
    }
  }
...
void Symbol::Compose(const array_view<const Symbol*>& args,
  ...
  if (IsAtomic(outputs)) {
    ...
    if (n_req != kVarg) {
      ...
      if (arg_vec.size() != n_req) {
        ...
        if (nmatched != kwarg_map.size()) {
          ...
          array_view<std::string> view(utils::BeginPtr(arg_names) + arg_vec.size(),
                                       utils::BeginPtr(arg_names) + arg_names.size());
    ...
    if (nmatched == kwargs.size() && arg_counter <= args.size()) {
      ...
      array_view<std::string> view(utils::BeginPtr(arg_names) + arg_counter,
                                   utils::BeginPtr(arg_names) + arg_names.size());
...
Symbol Symbol::GetInternals() const {
  ...
  DFSVisit(this->outputs, [&ret](const NodePtr& node) {
      ...
      if (n->is_variable()) {
        ...
        VariableParam& param = cvm::get<VariableParam>(n->attrs.parsed);
