...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *StateTrie) Prove(key []byte, fromLevel uint, proofDb ctxcdb.KeyValueWriter) error {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb ctxcdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof ctxcdb.KeyValueReader) (ctxcdb.KeyValueStore, *Trie, *KeyValueNotary, bool, error) {
