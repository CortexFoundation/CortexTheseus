...
func TestOneElementRangeProof(t *testing.T) {
	...
	tinyTrie, _ := New(TrieID(common.Hash{}), NewDatabase(rawdb.NewMemoryDatabase()))
...
func TestAllElementsEmptyValueRangeProof(t *testing.T) {
	...
	_, _, _, _, err := VerifyRangeProof(trie.Hash(), nil, nil, keys, vals, nil)
	if err == nil {
		t.Fatalf("Expected failure on noop entry")
	}
}
...
func TestRangeProofKeysWithSharedPrefix(t *testing.T) {
	...
	_, _, _, more, err := VerifyRangeProof(root, start, end, keys, vals, proof)
	if err != nil {
