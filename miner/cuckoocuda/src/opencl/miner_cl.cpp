...
namespace cuckoogpu {
	...
	cuckoo_hash() {
	    ...
	    for (node_t ui = (u >> IDXSHIFT) & CUCKOO_MASK;;
		 ui = (ui + 1) & CUCKOO_MASK)
	    {
		u64 old = cuckoo[ui];
	...
	uint32_t device;
	 ...
	 solver_ctx(const trimparams tp, uint32_t _device =
		    0, cl_context context =
		    NULL, cl_command_queue commandQueue =
		    NULL, cl_program program = NULL)
	{
	    ...
	    memcpy(headerBuf + 32,
		   static_cast < uint64_t * >(&littleEndianNonce),
		   sizeof (nonce));
	...
	void solution(const u32 * us, u32 nu, const u32 * vs, u32 nv) {
	    ...
	    cl_int clResult;
	    clResult =
		clEnqueueWriteBuffer(trimmer->commandQueue,
				     trimmer->recoveredges, CL_TRUE, 0,
				     sizeof (cl_uint2) * PROOFSIZE, soledges, 0,
				     NULL, NULL);
	    ...
	    int initV = 0;
	    clResult =
		clEnqueueFillBuffer(trimmer->commandQueue, trimmer->indexesE2,
				    &initV, sizeof (int), 0,
				    trimmer->indexesSize, 0, NULL, NULL);
	    if (clResult != CL_SUCCESS)
	    {
		printf("fill buffer error : %d\n", clResult);
	    }
	    clFinish(trimmer->commandQueue);
	    cl_kernel recovery_kernel =
		clCreateKernel(trimmer->program, "Recovery", &clResult);
	    clResult |=
		clSetKernelArg(recovery_kernel, 0, sizeof (cl_mem),
	    ...
	    clEnqueueNDRangeKernel(trimmer->commandQueue, recovery_kernel, 1,
				   NULL, global_work_size, local_work_size, 0,
				   NULL, &event);
	    clFinish(trimmer->commandQueue);
	    clResult =
		clEnqueueReadBuffer(trimmer->commandQueue, trimmer->indexesE2,
				    CL_TRUE, 0, PROOFSIZE * sizeof (u32),
				    &sols[sols.size() - PROOFSIZE], 0, NULL,
				    NULL);
	    if (clResult != CL_SUCCESS)
	    {
	    ...
	    {
		fprintf(stderr, "<%zu,%zu>, ", soledges[idx].x,
			soledges[idx].y);
	    }
	    ...
	    fprintf(stderr, "]\n");
	    qsort(&sols[sols.size() - PROOFSIZE], PROOFSIZE, sizeof (u32),
		  nonce_cmp);
	}
	...
	int solve() {
	    ...
	    cl_int clResult =
		clEnqueueReadBuffer(trimmer->commandQueue, trimmer->bufferB,
...
CuckooFindSolutionsCuda(uint8_t * header,
			...
			uint32_t * solLength, uint32_t * numSol)
{
    using namespace
	cuckoogpu;
    using
    ...
    {
	sprintf(headerInHex + 2 * i, "%02x",
		*((unsigned int8_t *) (header + i)));
    }
    headerInHex[64] = '\0';
    ...
    *solLength = uint32_t(sols[0].size());
    for (size_t n = 0;
	 n < std::min(sols.size(), (size_t) resultBuffSize / (*solLength)); n++)
    {
...
void
CuckooInitialize(uint32_t device)
{
    printf("thread: %d\n", getpid());
    using namespace
	cuckoogpu;
...
//    device = 0;
    ...
    cl_device_id
	deviceId = getOneDevice(platformId, device);
    if (deviceId == NULL)
	return;
    cl_context
