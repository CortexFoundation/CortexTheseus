...
#define DUCK_B_EDGES_NX (DUCK_B_EDGES * NX)
	...
	cl_int clResult;
        dipkeys = clCreateBuffer(this->context, CL_MEM_READ_WRITE, sizeof(siphash_keys), NULL, &clResult);
	...
	if(clResult != CL_SUCCESS){
        ...
        recoveredges = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_uint2) * PROOFSIZE, NULL, &clResult);
	if(clResult != CL_SUCCESS){
        ...
        bufferA = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeA, NULL, &clResult);
	if(clResult != CL_SUCCESS){
