...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, ctxc Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool, flashbots *flashbotsData) *worker {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	pending, err := w.ctxc.TxPool().Pending()
	if err != nil {
	...
	noBundles := true
	if w.flashbots.isFlashbots && len(w.ctxc.TxPool().AllMevBundles()) > 0 {
		noBundles = false
	}
	...
	if w.flashbots.isFlashbots {
		bundles, err := w.ctxc.TxPool().MevBundles(header.Number, header.Time)
		if err != nil {
			log.Error("Failed to fetch pending transactions", "err", err)
			return
		}
...
func (w *worker) mergeBundles(bundles []simulatedBundle, parent *types.Block, header *types.Header, pendingTxs map[common.Address]types.Transactions) (types.Transactions, simulatedBundle, int, error) {
	...
	quotaPool := core.NewQuotaPool(header.Quota)
	if err := quotaPool.SubQuota(header.QuotaUsed); err != nil {
		log.Warn("Quota pool create error", "err", err)
		return nil, simulatedBundle{}, 0, nil
	}
	...
	for _, bundle := range bundles {
		...
		simmed, err := w.computeBundleGas(bundle.originalBundle, parent, header, state, gasPool, quotaPool, pendingTxs, len(finalBundle))
...
func (w *worker) simulateBundles(bundles []types.MevBundle, coinbase common.Address, parent *types.Block, header *types.Header, pendingTxs map[common.Address]types.Transactions) ([]simulatedBundle, error) {
	...
	for _, bundle := range bundles {
		...
		simmed, err := w.computeBundleGas(bundle, parent, header, state, gasPool, quotaPool, pendingTxs, 0)
...
func (w *worker) computeBundleGas(bundle types.MevBundle, parent *types.Block, header *types.Header, state *state.StateDB, gasPool *core.GasPool, quotaPool *core.QuotaPool, pendingTxs map[common.Address]types.Transactions, currentTxCount int) (simulatedBundle, error) {
	...
	for i, tx := range bundle.Txs {
		...
		receipt, _, err := core.ApplyTransaction(w.chainConfig, w.chain, &w.coinbase, gasPool, quotaPool, state, header, tx, &tempGasUsed, *w.chain.GetVMConfig())
		if err != nil {
...
func ethIntToFloat(eth *big.Int) *big.Float {
	...
	return new(big.Float).Quo(new(big.Float).SetInt(eth), new(big.Float).SetInt(big.NewInt(params.Cortex)))
}
