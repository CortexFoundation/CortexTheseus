...
constexpr int XBITS = 7;

constexpr int YBITS = XBITS;

// size in bytes of a big bucket entry
#ifndef CUCKOO_BIGSIZE
#if EDGEBITS <= 15
#define CUCKOO_BIGSIZE 4
// no compression needed
#define CUCKOO_COMPRESSROUND 0
#else
#define CUCKOO_BIGSIZE 5
// YZ compression round; must be even
#ifndef CUCKOO_COMPRESSROUND
#define CUCKOO_COMPRESSROUND 14
#endif
#endif
#endif
// size in bytes of a small bucket entry
#define CUCKOO_SMALLSIZE CUCKOO_BIGSIZE

// initial entries could be smaller at percent or two slowdown
#ifndef CUCKOO_BIGSIZE0
#if EDGEBITS < 30 && !defined SAVEEDGES
#define CUCKOO_BIGSIZE0 4
#else
#define CUCKOO_BIGSIZE0 CUCKOO_BIGSIZE
#endif
#endif
// but they may need syncing entries
#if CUCKOO_BIGSIZE0 == 4 && EDGEBITS > 27
#define NEEDSYNC
#endif
...
const u32 BIGSLOTBITS   = CUCKOO_BIGSIZE * 8;
const u32 SMALLSLOTBITS = CUCKOO_SMALLSIZE * 8;
const u64 BIGSLOTMASK   = (1ULL << BIGSLOTBITS) - 1ULL;
const u64 SMALLSLOTMASK = (1ULL << SMALLSLOTBITS) - 1ULL;
const u32 BIGSLOTBITS0  = CUCKOO_BIGSIZE0 * 8;
...
#ifndef CUCKOO_BIGEPS
#define CUCKOO_BIGEPS 3/64
#endif

// 176/256 is safely over 1-e(-1) ~ 0.63 trimming fraction
#ifndef CUCKOO_TRIMFRAC256
#define CUCKOO_TRIMFRAC256 176
#endif

const u32 NTRIMMEDZ  = NZ * CUCKOO_TRIMFRAC256 / 256;

const u32 ZBUCKETSLOTS = NZ + NZ * CUCKOO_BIGEPS;
#ifdef SAVEEDGES
const u32 ZBUCKETSIZE = NTRIMMEDZ * (CUCKOO_BIGSIZE + sizeof(u32));  // assumes EDGEBITS <= 32
#else
const u32 ZBUCKETSIZE = ZBUCKETSLOTS * CUCKOO_BIGSIZE0; 
#endif
const u32 TBUCKETSIZE = ZBUCKETSLOTS * CUCKOO_BIGSIZE; 

template<u32 BUCKETSIZE>
struct zbucket {
  u32 size;
  // should avoid different values of RENAMESIZE in different threads of one process
  static const u32 RENAMESIZE = 2*NZ2 + 2*(CUCKOO_COMPRESSROUND ? NZ1 : 0);
  union alignas(16) {
    u8 bytes[BUCKETSIZE];
    struct {
#ifdef SAVEEDGES
      ...
      u32 renameu1[NZ2];
      u32 renamev1[NZ2];
      u32 renameu[CUCKOO_COMPRESSROUND ? NZ1 : 0];
      u32 renamev[CUCKOO_COMPRESSROUND ? NZ1 : 0];
#ifdef SAVEEDGES
      u32 edges[NTRIMMEDZ];
#endif
    };
...
// write        edge     YYYYYY    ZZZZZ
        *(BIGTYPE0 *)(base+dst.index[ux]) = zz;
        dst.index[ux] += CUCKOO_BIGSIZE0;
#else
        if (zz) {
          for (; unlikely(last[ux] + NNONYZ <= edge); last[ux] += NNONYZ, dst.index[ux] += CUCKOO_BIGSIZE0)
...
#define STORE0(i,v,x,w) \
  ...
  if (i || likely(zz)) {\
    ux = extract32(v,x);\
    for (; unlikely(last[ux] + NNONYZ <= edge+i); last[ux] += NNONYZ, dst.index[ux] += CUCKOO_BIGSIZE0)\
      *(u32 *)(base+dst.index[ux]) = 0;\
    *(u32 *)(base+dst.index[ux]) = zz;\
    dst.index[ux] += CUCKOO_BIGSIZE0;\
    last[ux] = edge+i;\
...
#error not implemented
    ...
    rdtsc1 = __rdtsc();
    if (!id) print_log("genUnodes round %2d size %u rdtsc: %lu\n", uorv, sumsize/CUCKOO_BIGSIZE0, rdtsc1-rdtsc0);
    tcounts[id] = sumsize/CUCKOO_BIGSIZE0;
  }
...
// print_log("id %d x %d y %d size %u read %d\n", id, ux, my, buckets[ux][my].size, readbig-base);
        for (; readbig < endreadbig; readbig += CUCKOO_BIGSIZE0) {
...
// print_log("id %d ux %d y %d e %010lx e' %010x\n", id, ux, my, e, ((u64)edge << ZBITS) | (e >> YBITS));
          small.index[uy] += CUCKOO_SMALLSIZE;
        }
...
// if (id==1) print_log("id %d ux %d y %d size %u sumsize %u\n", id, ux, uy, tbuckets[id][uy].size/CUCKOO_BIGSIZE, sumsize);
        for (u8 *rdsmall = readsmall; rdsmall < endreadsmall; rdsmall+=CUCKOO_SMALLSIZE)
...
// print_log("id %d ux %d y %d edge %08x e' %010lx vx %d\n", id, ux, uy, *readedge, uy34 | ((u64)(node & YZMASK) << ZBITS) | *readz, vx);
    ...
    rdtsc1 = __rdtsc();
    if (!id) print_log("genVnodes round %2d size %u rdtsc: %lu\n", uorv, sumsize/CUCKOO_BIGSIZE, rdtsc1-rdtsc0);
    tcounts[id] = sumsize/CUCKOO_BIGSIZE;
  }
...
// print_log("id %d vx %d vy %d size %u sumsize %u\n", id, vx, vy, tbuckets[id][vx].size/CUCKOO_BIGSIZE, sumsize);
...
// read           VXXXXXX     VYYYZZ'     UZZZZ   within UX UY partition  if !TRIMONV
          ...
          u16 vdeg = degs[vz];
// if (TRIMONV&&vx==75&&vy==83) print_log("id %d vx %d vy %d e %010lx e37 %x ux %x vdeg %d nrenames %d\n", id, vx, vy, e, e>>YZZBITS, ux, vdeg, nrenames);
          if (vdeg) {
            if (vdeg < 32) {
...
#ifdef CUCKOO_EXPANDROUND
#define CUCKOO_BIGGERSIZE CUCKOO_BIGSIZE+1
#else
#define CUCKOO_BIGGERSIZE CUCKOO_BIGSIZE
#define CUCKOO_EXPANDROUND CUCKOO_COMPRESSROUND
#endif
  void trimmer(u32 id) {
    genUnodes(id, 0);
    barrier();
    ...
    for (u32 round = 2; round < ntrims-2; round += 2) {
      barrier();
      if (round < CUCKOO_COMPRESSROUND) {
        if (round < CUCKOO_EXPANDROUND)
          trimedges<CUCKOO_BIGSIZE, CUCKOO_BIGSIZE, true>(id, round);
        else if (round == CUCKOO_EXPANDROUND)
          trimedges<CUCKOO_BIGSIZE, CUCKOO_BIGGERSIZE, true>(id, round);
        else trimedges<CUCKOO_BIGGERSIZE, CUCKOO_BIGGERSIZE, true>(id, round);
      } else if (round==CUCKOO_COMPRESSROUND) {
        trimrename<CUCKOO_BIGGERSIZE, CUCKOO_BIGGERSIZE, true>(id, round);
      } else trimedges1<true>(id, round);
      barrier();
      if (round < CUCKOO_COMPRESSROUND) {
        if (round+1 < CUCKOO_EXPANDROUND)
          trimedges<CUCKOO_BIGSIZE, CUCKOO_BIGSIZE, false>(id, round+1);
        else if (round+1 == CUCKOO_EXPANDROUND)
          trimedges<CUCKOO_BIGSIZE, CUCKOO_BIGGERSIZE, false>(id, round+1);
        else trimedges<CUCKOO_BIGGERSIZE, CUCKOO_BIGGERSIZE, false>(id, round+1);
      } else if (round==CUCKOO_COMPRESSROUND) {
        trimrename<CUCKOO_BIGGERSIZE, sizeof(u32), false>(id, round+1);
...
#define CUCKOO_NODEBITS (EDGEBITS + 1)
...
public:
  ...
  solver_ctx(const u32 n_threads, const u32 n_trims, bool allrounds, bool show_cycle, bool mutex_nonce) {
  ...
  u32 path(u32 u, u32 *us) const {
    ...
    for (nu = 0; u != CUCKOO_NIL; u = cuckoo[u]) {
      if (nu >= MAXPATHLEN) {
        ...
        else print_log("illegal %4d-cycle from node %d\n", MAXPATHLEN-nu, u0);
        pthread_exit(NULL);
      }
      us[nu++] = u;
    }
