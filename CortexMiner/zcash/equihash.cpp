...
void check_clEnqueueNDRangeKernel(cl_command_queue queue, cl_kernel k, cl_uint work_dim, const size_t *global_work_offset, const size_t *global_work_size, const size_t *local_work_size, cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event)
{
    cl_uint status;
...
// load kernel code file into a char* buffer
                           ...
                           size_t fixed_nonce_bytes, uint8_t *target, char *job_id)
{
    ...
    printf("%s ", s_hexdump(header + ZCASH_BLOCK_HEADER_LEN - ZCASH_NONCE_LEN +
                                fixed_nonce_bytes,
                            ZCASH_NONCE_LEN - fixed_nonce_bytes));
    printf("%s%s\n", ZCASH_SOLSIZE_HEX,
                     ...
                     char *job_id, uint32_t *shares)
{
    ...
    printf("preparing for verifying\n");
                        ...
                        uint32_t *shares)
{
    ...
    buf_blake_st = check_clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(blake.h), &blake.h);
                ...
                cl_kernel k_sols)
{
    ...
    if (!(dbg = calloc(dbg_size, 1)))
        fatal("malloc: %s\n", strerror(errno));
    buf_dbg = check_clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, dbg_size, dbg);
...
void equiSolver::runOpenCL()
{
    ...
    if (!(dbg = calloc(dbg_size, 1)))
        fatal("malloc: %s\n", strerror(errno));
    buf_dbg   = check_clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, dbg_size, dbg);
    buf_ht[0] = check_clCreateBuffer(context, CL_MEM_READ_WRITE, HT_SIZE, NULL);
    buf_ht[1] = check_clCreateBuffer(context, CL_MEM_READ_WRITE, HT_SIZE, NULL);
    buf_sols  = check_clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(sols_t), NULL);

    if (mining)
        mining_mode(context, queue, k_init_ht, k_rounds, k_sols, buf_ht,
                    buf_sols, buf_dbg, dbg_size, header.data());
    ...
    // Solve Equihash for a few nonces
    for (nonce = 0; nonce < nr_nonces; nonce++)
        total += solve_equihash(context, queue, k_init_ht, k_rounds, k_sols, buf_ht,
                                buf_sols, buf_dbg, dbg_size, header.data(), header_len, nonce,
                                0, NULL, NULL, NULL);
    uint64_t t1 = now();
    fprintf(stderr, "Total %" PRId64 " solutions in %.1f ms (%.1f Sol/s)\n",
            total, (t1 - t0) / 1e3, total / ((t1 - t0) / 1e6));
...
void equiSolver::getHeader(uint8_t* _header, int header_len){
    header.resize(140);
    if(header_len != 140 && header_len!=108){
        printf("unrecognized header length.\n");
        exit(0);
    }
    //copy
...
void equiSolver::getHeader(int header_len, const char* hex){
    header.resize(140);
    if(header_len != 140 && header_len!=108){
        printf("unrecognized header length.\n");
        exit(0);
    ...
    //for(int i=0; i<header_len; i++)
    //    header[i] = _header[i];
    parse_header(header.data(), header_len, hex);
}
