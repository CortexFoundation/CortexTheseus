...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.cortex", new NodeConfig());
			node.start();
			...
			CortexClient ec = node.getCortexClient();
	...
	// signatures too. Regression test for go-cortex issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altocortexer
	...
	workspace, err := ioutil.TempDir("", "cortex-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/cortex/cortextest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.cortex", "github.com/CortexFoundation/CortexTheseus/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "cortex.aar"), "cortex.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "cortex", "cortextest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
